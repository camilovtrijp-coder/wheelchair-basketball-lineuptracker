...
clubkleurcontrast, reduced-motion) geïmplementeerd, open als draft-PR #83
(nog niet gemerged). 8.2c (gedeeld apparaat, mobiele viewports, zwakke
verbinding) nog niet gestart. Zie §17-statustabel + `docs/pr-8.2-plan.md`.
...
| PR 8.2b — score-/wissel-/contextbediening, clubkleurcontrast en reduced-motion | Geïmplementeerd (open PR) | #83 | Voorbereidingsplan: `docs/pr-8.2-plan.md` §C 8.2b. Keyboard-bediening (score/wissel/kwart/contextwissel) bleek al volledig aanwezig — alleen e2e-bewijs toegevoegd (`a11y-keyboard.spec.ts`). Bug 10 (`docs/pr-5.5c-bugfixes.md` #10) opgelost via `--team-primary`/`--team-accent` CSS custom properties. **P1-review-opvolging (28 aug. 2026):** een eerste versie van de contrastfix toetste alleen tegen lichte-modus-vaste kleuren, waardoor `DEFAULT_SETTINGS`'s teamkleuren in donkere modus zelf onder de AA-drempel renderden (axe-core `color-contrast`) — `domain/settings/colorContrast.ts` herzien naar afgeleide, wiskundig gegarandeerd leesbare voorgrondkleuren (`deriveButtonForeground`/`deriveAccentForeground`, schema-bewust voor de headertitel); de niet-blokkerende waarschuwing uit de eerste versie is daardoor vervallen. `DEFAULT_SETTINGS.accentColor` vervangen (`#f97316` → `#c2410c`). Reduced-motion bevestigd met een nieuwe modal-intredeanimatie + regressietest (v2 had voordien geen enkele CSS-animation/transition). Lokaal tegen de Firebase-emulator: 937/937 unit-, 112/112 e2e- (incl. nieuwe `dark-mode-contrast.spec.ts`) en 66/67 auth-e2e-tests groen (de ene afwijking bleek een onafhankelijke sandbox-flake, bevestigd via herhaling) |
