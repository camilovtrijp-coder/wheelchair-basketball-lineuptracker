...
| Fase 4 — cloud/sync/tenancy-ADR's | In uitvoering |  | Firebase Auth + Firestore en Netlify blijven voorkeursroute; Supabase is alleen fallback na een gefaalde gate. ADR-001 (`docs/architecture/adr-001-cloud-data-platform.md`) is als concept opgesteld — regio/AVG/quota/prijsplan/back-ups/exit opnieuw geverifieerd (aug. 2026); nieuw AVG-aandachtspunt gevonden (Firebase Auth-metadata mogelijk buiten EU ondanks `eur3`-Firestore-data) en als openstaande vraag voor de eigenaar vastgelegd, nog niet stilzwijgend geaccepteerd. Formele acceptatie en PR 4.2/4.3 volgen na eigenaarsbeoordeling |
