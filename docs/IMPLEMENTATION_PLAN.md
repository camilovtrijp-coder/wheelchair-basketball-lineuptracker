...
| PR 5.5c-bugfixes — bugs gevonden tijdens handmatige staging-validatie | Gestart, verzamelfase (nog geen fixes toegepast) |  | Applicatiebugs die het handmatige 5.5c-protocol (`docs/pr-5.5-handmatig-protocol.md`) tegen de échte staging-Firebase (`wheelchair-basketball-tracker`) blootlegt, los van het `PR 6.1`-oplevermoment zelf: (1) `GameSetupPanel`/Wedstrijd-tab leidt `game` maar één keer af van `roster` (`App.tsx`'s `useEffect` draait alleen bij `game === null`) — een latere roster-wijziging binnen dezelfde sessie bereikt de Wedstrijd-tab niet zonder herladen; (2) "Opslaan" op Roster/Settings geeft geen zichtbare succes-/foutbevestiging (`RosterPanel.handleSave()` retourneert optimistisch vóór de Firestore-serverbevestiging). Details, reproductie en status per bug: `docs/pr-5.5c-bugfixes.md`. Meer bevindingen worden daar toegevoegd naarmate het protocol verder wordt uitgevoerd; de daadwerkelijke fixes (met regressietests) volgen in dezelfde PR |
...
| Fase 7 — wedstrijdsync en migratie | Implementatie gestart; PR 7.1a-c voltooid, 5.5b-activatie/5.5c-praktijkpoort in uitvoering (parallel, andere sessie), 7.2–7.4 nog niet gestart | #54, #55, #56 | Aanbevolen keten: voltooid fundament 5.3/5.4 → 7.1a-c → 5.5b-activatie/5.5c-praktijkpoort → 7.2a-c → 7.3a-c → 7.4a-c. Per §12-gatetekst: zonder de 5.5b/5.5c-poort mag emulatorontwikkeling van 7.2 al wél verder, alleen fase-7-acceptatie mag dan nog niet als voltooid gelden — 7.2a-c kan dus parallel aan de lopende 5.5c-handmatige validatie starten. Zie `docs/pr-7.1-plan.md` t/m `docs/pr-7.4-plan.md` |
