...
| PR 4.3 — ADR-003 tenancy/autorisatie | Concept |  | `docs/architecture/adr-003-tenancy-and-authorization.md`: datamodel (uid als membership-doc-ID), rollentabel, Rules-only-uitnodigingsflow zonder Cloud Function/Blaze (create/accept/claim in drie stappen met `email_verified`-mitigatie tegen self-grant), "laatste eigenaar niet Rules-afgedwongen" als bewust aanvaarde beperking in de application-laag. Openstaande vraag voor de eigenaar: akkoord met het Rules-only-ontwerp |
| Fase 4 — cloud/sync/tenancy-ADR's | In uitvoering | #23, #24 | ADR-001 en ADR-002 geaccepteerd; ADR-003-concept klaar ter beoordeling |
