...
| PR 8.2c — gedeeld apparaat, mobiele viewports en zwakke verbinding | Geïmplementeerd (open PR) |  | Voorbereidingsplan: `docs/pr-8.2-plan.md` §C 8.2c. `AuthGate.tsx`'s `handleSignOut()` breidt op het onvertrouwd-apparaatpad uit met `infrastructure/device/clearLocalDeviceData.ts` — een expliciete witte-lijst van `localStorage`-sleutels (geen `localStorage.clear()`/prefix-wildcard), incl. de `gameId`-gescoopte `game-sync-checkpoint`-sleutels van de actieve en voltooide wedstrijden van dat org/team; taalvoorkeur, vertrouwd-apparaatvlag en cloud-import-vlaggen blijven bewust staan. Nieuwe herroepbare vertrouwd-apparaat-instelling in `SessionBar.tsx` (checkbox + bevestigingsdialoog bij uitzetten) — wisselt live via `writeTrustedDevice()` + dezelfde wislogica, gevolgd door een paginareload (i.p.v. de bestaande `organizationGateway`/`repositories` live op een getermineerde Firestore-instantie te laten doorwerken). Tweede, kleiner/ouder mobiel viewportprofiel (375×667, iPhone SE 2016/8-formaat) in `tests/e2e/mobile-legacy.spec.ts` — **open besluitpunt (§B punt 6): deze viewportkeuze is een voorstel, vraagt expliciete bevestiging van de repo-eigenaar vóór merge.** CDP-netwerkemulatie (`tests/e2e-auth/game-sync-weak-network.spec.ts`, ~1500ms latency/3G-achtige bandbreedte) bewijst dat score-knoppen klikbaar blijven en snel-opeenvolgende score-acties in de juiste volgorde/zonder duplicatie aankomen tijdens een trage, niet-onderbroken sync. Lokaal: unit-, type-, lint- en buildcontroles groen (942 unit tests); e2e/e2e-auth niet lokaal uitvoerbaar (Playwright/Chromium-browserdownload is in deze sandbox netwerkgeblokkeerd, zelfde restpunt als 8.1c/7.3c/7.4c) — CI voert de nieuwe/gewijzigde e2e-specs uit |
