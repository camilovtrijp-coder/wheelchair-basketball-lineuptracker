...
Sub-PR's: 7.3a writerclaim/epoch/pre-game-gate; 7.3b live writer-sync en
read-only viewer; 7.3c overname/recovery/echte-apparaatvalidatie.
...
| Fase 5 — platformpilot settings/team | PR 5.1 t/m 5.4 voltooid; #27 gesloten op verkleinde scope (zie PR 5.3-rij), #28 gesloten in PR 5.1 | #31 | Firebase Emulator/Rules, Auth, multi-organisatiecontext, offline cache, twee apparaten en optionele Netlify-staging |
...
| Fase 7 — wedstrijdsync en migratie | Implementatie gestart; PR 7.1a voltooid, PR 7.1b in review, 7.1c/7.2–7.4 nog niet gestart |  | Aanbevolen keten: voltooid fundament 5.3/5.4 → 7.1a-c → 5.5b-activatie/5.5c-praktijkpoort → 7.2a-c → 7.3a-c → 7.4a-c. Zie `docs/pr-7.1-plan.md` t/m `docs/pr-7.4-plan.md` |
...
| PR 7.1b — Security Rules, queries, create-only actions |  |  | `firebase/firestore.rules` punt 8–12: `canWriteGameData(orgId,teamId)` (owner/admin/coach/scorer, spiegelt exact `v2/src/domain/organizations/teamAccess.ts` `deriveTeamAccess().canWriteGameData`); geneste `games/{gameId}`- en `actions/{actionId}`-Rules onder `teams/{teamId}`. Game-create: pad-/payloadcontext (`organizationId`/`teamId`), `phase in ['setup','tracking']`, `revision==0`, `writerEpoch==0`, en "maker"-borging (een initiële `writerUid` mag alleen de eigen `request.auth.uid` zijn — er is geen apart `createdBy`-veld op `GameDocument`). Game-update: strikte veldallowlist (alleen draaivelden/afgeleide snapshot/writer-/revisievelden) — kernvelden (`organizationId`/`teamId`/`players`/`opponent`/`competition`/`clockDown`/`limitStr`/`createdAt`) zijn hierdoor impliciet onveranderlijk; `phase` mag alleen gelijk blijven of `setup`→`tracking`; `writerEpoch` mag nooit dalen; `revision` moet exact met 1 omhoog (dit ís de optimistische-concurrencycontrole voor PR 7.1c's snapshotpatches, geen aparte transactie nodig). Action-create: create-only (update/delete altijd `false`), eigen auteur (`authorUid==request.auth.uid`), en `writerEpoch`/`deviceId` moeten overeenkomen met de ACTUELE claim (`writerUid`/`deviceId`/`writerEpoch`) op het parentdocument — een actie met een oude/overgenomen epoch wordt hierdoor al geweigerd, vooruitlopend op de daadwerkelijke overnamelogica uit PR 7.3. Bewust GEEN recursieve-wildcard match voor `games`/`actions` (in tegenstelling tot `organizationMembers`/`teamMembers`): er is nog geen cross-team-querybehoefte, dus elke `collectionGroup`-query blijft default-deny — vastgelegd in `firebase/docs/QUERY_CONTRACT.md` §"Wedstrijd-/actiepaden", geen nieuwe `firestore.indexes.json`-overrides nodig. Nieuwe `tests/rules/games-and-actions.spec.ts` (39 tests): volledige rolmatrix (owner/admin/coach/scorer mogen schrijven, viewer alleen lezen) voor create/read/update/delete van zowel games als actions, cross-org/team-isolatie, self-promotion (vervalste `writerUid`/contextvelden bij create), vervalste auteur, stale-writerEpoch- en afwijkende-deviceId-weigering, action-update/delete altijd geweigerd (ook door de auteur/owner zelf), en een queryscope-test die bewijst dat een `collectionGroup('actions')`-query default-deny blijft terwijl het directe pad gewoon leesbaar is. `sampleGame()`/`sampleGameAction()` toegevoegd aan `tests/rules/helpers/fixtures.ts`. Volledige `firebase-base`-Rules-suite blijft groen (9 bestanden, 106 tests, was 67); unit-suite ongewijzigd (57 tests). `firebase-spike/` (eigen, onafhankelijke `firestore.rules`) niet aangeraakt. Geen wijziging aan `v2/`. |
