...
| Fase 7 — wedstrijdsync en migratie | Implementatie gestart; PR 7.1a–7.1c voltooid, PR 7.2a in review, 7.2b–7.4 nog niet gestart. Praktijkpoort 5.5c (echte iOS/Android-offline-reload + billable Firestore-baseline) nog niet uitgevoerd — 7.2a is dus emulatorontwikkeling conform het plan ("zonder deze poort mag emulatorontwikkeling verder, maar fase-7-acceptatie niet als voltooid worden gemarkeerd") |  | Aanbevolen keten: voltooid fundament 5.3/5.4 → 7.1a-c → 5.5b-activatie/5.5c-praktijkpoort → 7.2a-c → 7.3a-c → 7.4a-c. Zie `docs/pr-7.1-plan.md` t/m `docs/pr-7.4-plan.md` |
