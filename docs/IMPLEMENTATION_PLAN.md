...
| PR 4.2 — ADR-002 offline-sync | Concept |  | `docs/architecture/adr-002-offline-sync-strategy.md`: syncstatuscontract (4 toestanden), append-only wedstrijdacties met clientgegenereerde UUID's i.p.v. één overschreven megadocument, tombstones, single-writer-model, pre-game-offline-readinesscheck. Browserondersteuning persistente cache (Chrome/Safari/Firefox) herbevestigd; single-tab vs. multi-tab persistence expliciet als openstaande vraag voor de eigenaar |
| Fase 4 — cloud/sync/tenancy-ADR's | In uitvoering | #23 | ADR-001 geaccepteerd; ADR-002-concept klaar ter beoordeling; PR 4.3 (ADR-003, tenancy) volgt |
