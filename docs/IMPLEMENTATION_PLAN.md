...
| PR 4.4 — begrensde Firebase-spike | Voltooid | — | `firebase-spike/` zelfstandige workspace; Firebase Emulator Suite (Firestore :8080, Auth :9099), fictieve data, geen echt GCP-project. Bewijst: offline edit/reload/reconnect/tweede cliënt (Playwright); volledige rol- en organisatie-isolatiematrix (Vitest + rules-unit-testing); Rules-only uitnodigingsflow (create/accept/claim); intrekking-tijdens-queued-write (Node- en browser-variant). Kritieke architectuurvondst: sync-poorten incompatibel met async Firestore — async sibling-ports geïntroduceerd, migratie naar productie-UI uitgesteld naar PR 5.3. Alle harde beslisgates gepasseerd; **go voor Fase 5**. Zie `firebase-spike/docs/SPIKE_REPORT.md` |
| Fase 4 — cloud/sync/tenancy-ADR's | Voltooid | #23, #24, #25 | ADR-001, ADR-002 en ADR-003 alle geaccepteerd en empirisch getoetst via PR 4.4. Volgende stap: Fase 5 (Firebase-platformfundament en multi-organisatiepilot) |
