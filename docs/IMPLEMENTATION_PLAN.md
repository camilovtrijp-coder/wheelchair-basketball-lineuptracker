...
| PR 4.4 — begrensde Firebase-spike | Herreviewcorrectie verwerkt | — | `firebase-spike/` zelfstandige workspace; Firebase Emulator Suite (Firestore :8080, Auth :9099), fictieve data, geen echt GCP-project. Bewijst: offline edit/reconnect/tweede cliënt (Playwright); volledige rol- en organisatie-isolatiematrix (Vitest + rules-unit-testing); Rules-only uitnodigingsflow (create/accept/claim); intrekking-tijdens-queued-write (Node- en browser-variant). Kritieke architectuurvondst: sync-poorten incompatibel met async Firestore — async sibling-ports geïntroduceerd, migratie naar productie-UI uitgesteld naar PR 5.3. Eerste correctieronde: admin→owner-beveiligingsgat hersteld, intrekkingstest strikt, ongecachete offline-test. Tweede correctieronde: TS2532-fix, replay-blokkade uitnodiging (`claimed`-status), subscribe()-DEFAULT_SETTINGS-fix, conditioneel Chromium-pad. Herreviewcorrectie: membership + invitation-claim atomair afgedwongen, type-check in `spike:verify`, volledige spike als aparte CI-job. 45 rules + 3 e2e = 48 tests. **Herreview vereist vóór go/no-go Fase 5.** Zie `firebase-spike/docs/SPIKE_REPORT.md` |
| Fase 4 — cloud/sync/tenancy-ADR's | Voltooid (PR 4.4 herreview lopend) | #23, #24, #25 | ADR-001, ADR-002 en ADR-003 alle geaccepteerd en empirisch getoetst via PR 4.4. Start Fase 5 na afronding herreview PR 4.4 |
