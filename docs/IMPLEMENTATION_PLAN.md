...
Sub-PR's: 7.3a writerclaim/epoch/pre-game-gate (**geïmplementeerd**, zie
§17-statustabel + `docs/pr-7.3-plan.md` §C 7.3a); 7.3b live writer-sync en
read-only viewer (**geïmplementeerd**, zie §17-statustabel +
`docs/pr-7.3-plan.md` §C 7.3b); 7.3c overname/recovery/echte-
apparaatvalidatie (nog niet gestart — de overname-UI/-bevestigingsflow en de
echte-apparaat-e2e-validatie voor claimrace/contextwissel horen hier).
...
Sub-PR's: 7.4a inventarisatie/mapping/preview; 7.4b hervatbare
migratiecoordinator; 7.4c migratie-UI en volledige e2e.
...
| Fase 7 — wedstrijdsync en migratie | Implementatie gestart; PR 7.1a–7.1c, 7.2a en 7.2b gemerged/geïmplementeerd, PR 7.2c geïmplementeerd, 7.3–7.4 nog niet gestart. Praktijkpoort 5.5c (15-16 aug. 2026) gepasseerd: Android-kant volledig uitgevoerd (2/2 schoon, echte billable Firestore-baseline gemeten, zie `docs/pr-5.5-onderzoeksrapport.md`); iOS-kant expliciet, apart openstaand (geen Apple-apparaat beschikbaar bij de eigenaar) — de eigenaar heeft op 16 aug. 2026 besloten dat dit de sluiting van 5.5's acceptatiefase (en daarmee deze poort) niet blokkeert, met iOS als losstaand bijgehouden punt |  | Aanbevolen keten: voltooid fundament 5.3/5.4 → 7.1a-c → 5.5b-activatie/5.5c-praktijkpoort → 7.2a-c → 7.3a-c → 7.4a-c. Zie `docs/pr-7.1-plan.md` t/m `docs/pr-7.4-plan.md` |
...
| PR 7.3a — claim, epoch en pre-game gate | Geïmplementeerd | | Voorbereidingsplan: `docs/pr-7.3-plan.md` §C 7.3a (volledig implementatieverslag daar — hier alleen de samenvatting). Nieuw `domain/game/writerClaim.ts` (pure claim-/epochtypes + `gameStartBlockReason()`, combineert roster- en cloudclaim-redenen). `firestore.rules`/`firebase/src/documents/game.ts`: nieuwe velden `claimedAt`/`lastWriterActivityAt`; nieuw pad 10d (**overname**) — elke bevoegde rol mag een AL geclaimd document overnemen mits `writerEpoch` met exact 1 omhoog gaat, geen `runTransaction()` nodig (Firestore serialiseert per document, Rules herevalueren tegen de laatste staat — zelfde garantie). `GameCloudGateway.claimWriter()`/`takeoverWriter()` (nieuw, met foutclassificatie naar `WriterClaimErrorCode`); `GameSyncCoordinator` gebruikt voortaan het ECHTE serverepoch voor action-uploads (was vóór 7.3a statisch op 0) en krijgt `ensureWriterClaim()` (het expliciete, blokkerende claimpad vóór tip-off) en `takeoverWriter()` (bouwsteen, nog geen UI-knop — dat is 7.3c-scope). `GameSetupPanel`/`App.tsx`: de startknop blijft in cloud-modus geblokkeerd tot een serverbevestigde claim (`cloudClaim` state, automatisch aangevraagd zodra de wedstrijd startbaar is), met een NL/EN-herstelmelding per foutcode en een "Opnieuw proberen"-knop; alleen-lokale modus blijft ongewijzigd zonder claim/netwerk. `App.tsx`/`AuthGate.tsx`: nieuwe `onGameLockChange`-prop — een bevestigde claim OF `phase === 'tracking'` blokkeert `handleBackToSwitcher()` met een dismissible banner (geen "expliciet loslaten"-actie nog, dat is 7.3c-scope). **Tests:** firebase — 209 rules-tests (was 189: het volledige 10d-overnamepad — geldig, self-promotion, viewer, epoch-sprong, epoch-ongewijzigd, lege deviceId, claimedAt/lastWriterActivityAt-niet-samen, draaivelden-in-dezelfde-patch, stale revision, afgeronde wedstrijd, en het fencingbewijs dat een oude actie na overname geweigerd wordt), 75 unit-tests; volledige rules-/unit-suite groen tegen de emulator. v2 — nieuw `writerClaim.spec.ts`, uitgebreid `GameSyncCoordinator.spec.ts` (`ensureWriterClaim()`/`takeoverWriter()`-scenario's), nieuw `GameSetupPanel.spec.tsx` (alle claimstatussen/foutcodes), nieuw `AppWriterClaim.spec.tsx` (end-to-end door `App` heen: automatische claim, lock-timing, alleen-lokale modus blijft claimloos); volledige v2-suite nu 713 tests, typecheck, eslint groen. **Nog niet gedaan:** overname-bevestigings-UI en echte-apparaat-/emulator-e2e-tests voor claimrace/contextwissel zijn bewust doorgeschoven naar 7.3c (§C 7.3c werk 1/4/5); 7.3b (live writer-sync, read-only viewer) is inmiddels geïmplementeerd, zie de volgende rij. |
| PR 7.3b — live writer-sync en read-only viewer | Geïmplementeerd | | Voorbereidingsplan: `docs/pr-7.3-plan.md` §C 7.3b (volledig implementatieverslag daar — hier alleen de samenvatting). Werk 1 (epoch/sequence/idempotente upload) bleek al volledig aanwezig sinds PR 7.1c/7.3a; deze PR is dus puur de LEESKANT plus een teambrede single-writer-gate die tijdens implementatie nodig bleek. Nieuw `application/game/liveView.ts` (puur, bewust niet in `domain/game/` — dat moet dependency-vrij blijven per `firebase-spike/tsconfig.json`, wat een eerste, verkeerd geplaatste versie van dit bestand ook daadwerkelijk in CI liet falen vóór deze push): `deriveLiveGameActions()`/`buildLiveGameView()` reconstrueren een read-only `ActiveGame`-weergave uit een cloud-`GameDocument` + `actions`-envelopes (dedupliceert op `actionId`, sorteert op `(writerEpoch, sequence, actionId)`), `pickActiveGameCandidate()` kiest de meest recent actieve kandidaat. Nieuwe `application/game/GameViewerGateway.ts`-poort + `infrastructure/game/FirestoreGameViewerGateway.ts`: een tweetraps-abonnement — (1) een discoveryquery binnen één team (`games`-subcollectie, `phase=='tracking' && completedGameId==null`, geen `orderBy`, dus geen nieuwe Firestore-index nodig; geen Rules-wijziging nodig, valt al onder de bestaande `canReadTeam`-leesregel, geen `collectionGroup()`), (2) een geneste parent+actions-abonnementwissel zodra de gevonden `gameId` verandert. **Bug gevonden tijdens de eigen emulator-e2e-verificatie** (vóór merge, dus geen productie-impact): de discoveryhandler vergeleek de gekozen `gameId` met een op `null` geïnitialiseerde vorige waarde — het EERSTE snapshot van een lege collectie (`chosen===null`) leek zo gelijk aan de initiële staat en riep nooit `emit()` aan, waardoor "geen actieve wedstrijd" nooit gemeld werd; gefixt met een apart `undefined`-sentinel voor "nog geen snapshot verwerkt". **Teambrede single-writer-gate** (niet vooraf gepland, bleek nodig): `ActiveGame.id` wordt per apparaat lokaal gegenereerd, dus zonder ingrijpen kon apparaat B tijdens de tab-navigatie zijn EIGEN, andere `gameId` claimen/starten terwijl apparaat A al actief was — twee gelijktijdig actieve wedstrijden voor hetzelfde team. `App.tsx`: nieuwe `activeCloudGame`-state gevoed door `repositories.gameViewer` (`null` lokaal), en een nieuwe `viewerBlocksClaim`-voorwaarde op de bestaande 7.3a-`ensureWriterClaim()`-auto-effect (blokkeert zolang er nog geen definitief viewerantwoord is ÓF een andere schrijver actief is) — inclusief een gerichte racefix (`activeCloudGame` expliciet terug naar `null` bij elk nieuw abonnement, een listenerfout meldt expliciet `'none'` i.p.v. `null`) nadat de eigen `AppLiveViewer.spec.tsx` de race reproduceerde. `LiveTrackingPanel` kreeg een optionele `liveViewerSync`-prop (vervangt de generieke read-only-banner door een specifiekere melding + `SyncStatusIndicator`); de render-tak in `App.tsx` toont bij `activeCloudGame?.kind==='active'` dezelfde `LiveTrackingPanel` met `canWrite={false}` (apparaat-/writerclaim-fencing, los van de rol-`canWriteGame`). **Tests:** v2 — nieuw `liveView.spec.ts` (14 tests: actietype-roundtrips, out-of-order/duplicated-envelope-afhandeling, epoch-ordering, kandidaat-tiebreaks), nieuw `AppLiveViewer.spec.tsx` (4 tests, gescripte gateway: read-only weergave zonder claimpoging, terugval-met-claim bij `'none'`, listenerfout valt terug op `GameSetupPanel`, alleen-lokale modus abonneert nooit); volledige v2-unit-suite nu 735 tests. Nieuwe emulator-e2e `tests/e2e-auth/game-viewer-second-client.spec.ts` (een onafhankelijke tweede Firestore-client, `FirestoreGameViewerGateway` rechtstreeks aangeroepen — geen volledige tweede-browser-UI nodig): bewijst discovery zonder vooraf de `gameId` te kennen, live updates zonder reload, "geen actieve wedstrijd" vóór tip-off én ná afronden, en de kern van 7.3b's acceptatie — de viewer blijft op de laatst servergesynchroniseerde stand terwijl apparaat A offline doorscoort, met convergentie ná reconnect. Volledige `test:e2e:auth`-suite (60 specs, was 59) en `test:e2e`-suite (90 specs) groen tegen de echte Firebase-emulator; typecheck/eslint/prettier/build groen. **Nog niet gedaan:** de sterke overname-bevestigingsflow/UI-knop en echte-apparaatvalidatie (twee browsers/iOS/Android) blijven 7.3c-scope; de writerbanner blijft bewust generiek ("een ander apparaat", geen displaynaam-lookup — zelfde precedent als `claimBlockedAlreadyClaimed`). |
