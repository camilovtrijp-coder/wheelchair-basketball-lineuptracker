...
| PR 5.4 — multi-organisatie- en twee-apparatenpilot | 5.4a (commit 494b4d9), 5.4b (#40) en 5.4c (deze PR) voltooid; alle §10-acceptatiecriteria voor 5.4 bewezen | 5.4a, 5.4b (#40), 5.4c | **5.4a — UI-rolgrenzen + listener-fout-detectie:** `canWrite`-prop door `AuthGate` afgeleid uit dezelfde `validateSelectedTeam()`-call als `selectedContextTeamValid` (geen extra Firestore-read, geen disabled-flash voor owners/admins); `SettingsPanel`/`RosterPanel` krijgen `canWrite=false` voor scorer/viewer met disabled schrijfknoppen, readOnly speler-inputs en "Alleen-lezen"-indicator (`settingsReadOnly`/`rosterReadOnly`, NL+EN); `App.tsx`'s nieuwe `listenerError`-state met niet-blokkerende "Verbinding met cloud weggevallen"-indicator voor post-load-fouten, pre-load-fouten blijven naar `OfflineUncachedScreen`. Lost de §J-follow-up #1 uit PR #36 op vóór de pilot. Tests: `v2/tests/e2e-auth/role-matrix-ui.spec.ts` (5 rollen × 2 panels, 10 e2e-tests) en `v2/tests/unit/AppListenerError.spec.tsx` (controlled mock-repository, 2 cases). **5.4b — twee apparaten, conflict en actualiteit:** `FirestoreSettingsRepository.write()` patcht uitsluitend de gewijzigde velden via `setDoc(..., { merge: true })` zodra `documentExists` en `changedKeys.length > 0`; nieuwe documenten krijgen altijd een volledige schemawrite; lege `changedKeys` op een bestaand document is een no-op. `FirestoreRosterRepository.write()` blijft volledig. Last-write-wins op hetzelfde veld volgt Firestore-standaardsemantiek — bewust geen "wie won"-attributie, alleen `updatedAt`. Vier nieuwe e2e-tests tegen de Firebase-emulator: `two-devices-same-context.spec.ts` (live-deliver), `two-devices-different-orgs.spec.ts` (org-isolatie), `deliberate-conflict.spec.ts` (LWW), `concurrent-non-conflicting-writes.spec.ts` (merge). UI: `LastModified` met serverbevestigde `updatedAt` en `fromCache`-sublabel in `SyncStatusIndicator` ("uit cache"/"from cache"). Extra unit-tests: `FirestoreSettingsRepository.spec.ts` (field-patch + retry-pad), `useSyncStatus.spec.ts` (18 cases, opvolging reviewpunten 2-5 uit PR #36), `LastModified.spec.tsx`, `SyncStatusIndicator.spec.tsx`. **5.4c — pilot-rapport en §17-correctie (deze PR):** `docs/pr-5.4-onderzoeksrapport.md` uitgebreid volgens `pr-5.4-plan.md` §C/5.4c stramien (§A scope + zeven locked-in beslissingen, §B scenario-bevindingen-tabel, §C open punten naar 5.5, §D cross-references); §17-tabel in dit bestand gecorrigeerd (Fase 4-rij, PR 4.4-rij en Fase 5-rij verwijzen niet meer naar #28 als open gate). Geen code, geen tests, geen i18n-, localStorage- of CSV-aanraking. §J-trigger-criteria in `pr-5.3d-onderzoeksrapport.md` regels 430-461 (commit `1957182`) en #28-sluiting (PR #29) waren al uitgevoerd |
