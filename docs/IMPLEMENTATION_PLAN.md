...
Laatst gevalideerd: `main` op commit `15bc9117a305d65e5528c94807e19dcb255867ab`
...
**Voltooid**: 8.2a (focus-infrastructuur, keyboard-navigatie,
axe-core-baseline) gemerged (#81), 8.2b (score-/wissel-/contextbediening,
clubkleurcontrast, reduced-motion) gemerged (#83) en 8.2c (gedeeld apparaat,
mobiele viewports, zwakke verbinding) gemerged (#84) — herroepbare
...
**8.3a volledig gemerged via PR #85 (squash-commit `f5412cc`).**
Zie [`docs/pr-8.3-plan.md`](pr-8.3-plan.md) en
[`docs/security-threat-model.md`](security-threat-model.md). De overige scope
blijft gesplitst in 8.3b (organisatie-export/herstelbewijs), 8.3c
(bewaartermijnen en verwijdering achter expliciete eigenaarsbesluiten) en 8.3d
(kosten/back-up/platformbeheer).
...
| PR 8.2c — gedeeld apparaat, mobiele viewports en zwakke verbinding | Gemerged | #84 | Voorbereidingsplan: `docs/pr-8.2-plan.md` §C 8.2c. Herroepbare vertrouwd-apparaatinstelling met expliciete, allowlisted apparaatdata-opruiming; tweede 375×667-viewport; zwakke-netwerk-e2e die de pending-syncstatus vóór een tweede actie waarneemt. Exact-headreview en CI waren groen vóór merge. |
| PR 8.3a — securitybaseline, App Check-monitoring en lokale diagnostiek | Gemerged | #85 | Dreigingsmodel + machineleesbare Rules-/gatewaymatrix; exacte parentdocument-shapes en geen client-hard-delete; opt-in App Check voor staging/productie zonder enforcement/debugprovider; begrensde lokale allowlistdiagnostiek met NL/EN-export. Volledige Firebase-suite, v2-unit/lint/build en gerichte emulator-browserprobe groen. Herreview-iteraties: matrix-completeness in beide richtingen (Rules-matches, gatewaybestanden, converters — vóórdien manueel onderhouden en dus stil kunnen verouderen), platformonafhankelijke padnormalisatie (Windows-backslashes), en PR-statusregels bijgewerkt. |
| PR 8.3b — volledige organisatie-export en herstelbewijs | Gemerged in twee delen | #87, #89 | Voorbereidingsplan: `docs/pr-8.3-plan.md` §C 8.3b (volledig implementatieverslag daar). **Deel 1/2 (#87):** `domain/export/types.ts` met het versieerbare `OrganizationExportV1`-contract en `canExportOrganization()` — een bewust smallere allowlist dan `canBulkMigrate()`, want een volledige organisatie-export bevat alle membership-e-mails en mag niet stilzwijgend meeveranderen; pure `buildOrganizationExport()` (hergebruikt `payloadHash()` uit de migratielaag, geen tweede hashformaat) en `verifyOrganizationExportRoundtrip()`; `OrganizationExportGateway`-poort plus coordinator die de capabilitycheck vóór élke Firestore-read doet, zodat een niet-owner geen enkele read veroorzaakt; `FirestoreOrganizationExportGateway` leest alle elf gegevensfamilies over alle teams en faalt in zijn geheel bij één mislukte read (geen gedeeltelijk resultaat). Nieuwe Rules-test `organization-export-listing.spec.ts` bewijst dat de nieuwe queryvorm — een ongefilterde `getDocs(collection(...))`-listing — onder de bestaande Rules exact hetzelfde autorisatiegedrag geeft als een losse `getDoc()`. **Deel 2/2 (#89):** `ui/export/ExportPanel.tsx` (owner-only NL/EN-preview met aantallen per gegevensfamilie en gevoelige-inhoudwaarschuwing, wiring via `App.tsx` alleen in cloudmodus), bestandsnaam- en downloadadapter gespiegeld van de bestaande back-uplaag, plus `organization-export-flow.spec.ts` (rolgating voor alle vijf rollen, en een stroom met twee organisaties die bewijst dat de export van organisatie A geen enkel gegeven van B bevat) en `organization-export-restore-proof.spec.ts` met een test-only Admin-/Emulatorharness die een export terugschrijft naar een geheel nieuwe organisatie en via de echte Rules een inhoudelijk gelijke inventaris terugleest — de bron blijft daarbij byte-voor-byte intact (herhaalde bronexport geeft dezelfde `contentHash`). **Drie herreviewrondes vóór merge, alle drie op autorisatie:** (1) `callerRole` was een door de aanroeper meegegeven React-state en dus geen betrouwbare grens — vervangen door een echte lezing van `organizationMembers/{uid}`; (2) de restoreproef verwarde de oprichter (`organization.createdBy`) met de actueel exporterende eigenaar (`exportedBy`), waardoor een overgedragen eigendom de doelaccount geen membership gaf; (3) het zwaarste punt — `callerUid` bleef als parameter staan, zodat een `organizationAdmin` simpelweg de uid van de owner kon meegeven en die read alsnog slaagde (Rules staan elk orglid toe elk `organizationMembers`-document binnen de organisatie te lezen). Opgelost door de identiteitsparameter volledig te schrappen: `readAuthoritativeCaller(organizationId)` haalt de uid uitsluitend uit `getAuth(db.app).currentUser`, dus geen enkel veld op de request draagt nog een identiteit die de aanroeper zelf kan invullen. Een e2e met een echt aangemaakte admin bewijst dat, zonder mocks. **Vastgelegd architectuuronderscheid (niet-blokkerend, commit `14b6f58`):** dit maakt de export owner-only als *productcapability*, niet als *server-side vertrouwelijkheid tegenover admins* — die rol houdt via `isOrgMember()` legitieme leestoegang tot dezelfde onderliggende paden en zou met eigen clientcode een vergelijkbare dataset kunnen samenstellen. Die twee mogen niet door elkaar gebruikt worden in latere acceptatieclaims; zou het laatste ooit een harde eis worden, dan vraagt dat een apart Rules-/architectuurbesluit, geen clientpatch. **Verificatie:** v2 995/995 unit-tests (was 955 vóór 8.3b), `tsc -b`/eslint/prettier over de volledige `src`+`tests`-boom en de productie-/classic-SW-build groen; Firebase 86/86 unit-/convertertests en 237/237 Emulator Rules-tests (was 232), `type-check` groen. **Twee heads, bewust onderscheiden:** `485c39e` is de laatste head waarop de CODE is gereviewd (de commit die `callerUid` verwijderde) — CI-run 34265843465 daarop 4/4 groen, alle code-bevindingen opgelost; `14b6f58` is de uiteindelijk gemergede head en uitsluitend een DOCUMENTATIEcommit (het architectuuronderscheid hierboven) — CI-run 34268193184 daarop eveneens 4/4 groen, op run-attempt 2. Een acceptatieclaim over de 8.3b-code hoort dus naar `485c39e` te verwijzen, niet naar `14b6f58`: die laatste voegde geen regel code toe. De twee nieuwe Playwright-e2e-auth-bestanden konden in die sessies niet lokaal draaien (de sandbox blokkeerde het downloaden van de emulator-jars) en zijn via de CI-jobs geverifieerd — vanaf PR 8.3c-0 kon de emulator wél lokaal draaien |
| PR 8.3c-0 — servergebonden bewaartijdstempels | Gemerged | #91 | Besluitrecord: `docs/pr-8.3c-besluitvoorstel.md` §3.1/§5 (gemerged via #90, dat de §E.1–E.3-startblokkade ophief en 8.3c in drie PR's knipte). Eerste van die drie, en bewust afgesplitst omdat dit als enige onderdeel bestaande, werkende schrijfpaden en hun fixtures raakt. `firestore.rules`: vijf eisen — `invitations` create bindt nu `invitedAt == request.time` (er stond hiervoor géén enkele eis op dat veld, niet eens dat het bestond) en eist `acceptedAt == null`; de accepteerpatch eist `acceptedAt == request.time` en de claimpatch `claimedAt == request.time` (de `hasOnly()`-allowlist liet de WAARDE van beide velden eerder volledig vrij); de tombstonepatch op `completedGames` gaat van `deletedAt is timestamp` naar `deletedAt == request.time`. Volgt het patroon dat `organizations`/`teams` al voor `createdAt` gebruiken. **Waarom dit nodig was:** elke bewaartermijn die 8.3c-1 op deze velden legt, rekende anders met een tijdstempel die de aanroeper zelf invult — een ondergrens die terugdateren toelaat is geen grens. Dat gold nadrukkelijk óók voor de 90-dagengrens op tombstones, wat bij de eerste review niet was opgemerkt. **Voor de app een no-op:** `acceptInvitation()`/`claimInvitation()` (`FirestoreOrganizationGateway.ts`) en `tombstoneCompletedGame()` (`FirestoreGameCloudGateway.ts`) schreven deze velden al met `serverTimestamp()`; het werk zat in de testfixtures, die kale `new Date()`-waarden schreven. Nieuw `firebase/tests/rules/server-bound-timestamps.spec.ts` (25 tests): per veld het positieve geval plus ontbrekend, verkeerd getypeerd, in de toekomst en teruggedateerd, en voor `acceptedAt` bij create de null-variant. In `bootstrap-and-invitation-flow.spec.ts` schrijven alle 18 CLIENT-writes nu `serverTimestamp()` — óók die welke moeten falen, zodat ze blijven falen om hun eigen reden (verkeerde e-mail, niet-geverifieerd, replay) en niet stilzwijgend om een afgekeurde tijdstempel; de `withAdmin()`-seeds houden bewust `new Date()`, want die omzeilen Rules en stellen bestaande documenten voor. **Verificatie, lokaal tegen de echte Firestore-/Auth-emulator** (die in deze sessie voor het eerst in deze reeks kon draaien): firebase 87/87 unit-/convertertests en 262/262 Rules-tests (was 237/237), `type-check` groen; v2 ongewijzigd op 995/995 unit-tests met `tsc -b`/eslint groen; alle vier CI-jobs groen op exact head `b7e8a69`. De atomaire `accepted → claimed`-batch, de replayweigering en de legacy-tombstoneflow (document zonder `revision`/`deletedAt`/`deletedBy`) zijn expliciet opnieuw bewezen — die drie waren het grootste regressierisico. Beide fixes zijn vóór de wijziging gereproduceerd: met de Rules teruggedraaid faalden 14 van de 20 tijdstempeltests, en met alleen de `acceptedAt`-tests toegevoegd precies die 4 — geen van de nieuwe tests is dus een false positive. **Open pre-8.5-poort** (vastgelegd in `docs/security-threat-model.md` §7, niet blokkerend voor 8.3c-0): `invitedAt == request.time` bewijst alleen iets over Rules-gecontroleerde writes, en er bestaat nog geen uitnodigingsaanmaakpad in `v2/src` — `FirestoreOrganizationGateway` kent alleen read/accept/claim, en de handmatige Console-write uit het stagingprotocol loopt via Admin-privileges en omzeilt Rules volledig. Vóór echte spelersdata moet het gezaghebbende productie-aanmaakpad worden vastgesteld; dat is een productbesluit, geen codefix. Niet in deze PR: `revokedAt`, `redactedAt`, het collectionGroup-querycontract en de index voor `invitations.email`, de deleteflow en elke UI — die blijven 8.3c-1 en 8.3c-2 |
