...
- `v2/tests/e2e-auth/listener-error-indicator.spec.ts` _(stretch-goal, optioneel)_ — forceer een listener-fout na initiële load (bijv. `page.route('**/*', route => route.abort())` op emulator-verkeer na een bepaald moment). Alleen opnemen als de strategie in CI stabiel blijkt.
...
1. **Vier nieuwe e2e-tests in `v2/tests/e2e-auth/`:** Elke test registreert één of twee unieke gebruikers en seedt de benodigde organisaties, teams en memberships via `adminDb()` (hetzelfde patroon als `team-level-authorization.spec.ts`).
   ...
   - **`concurrent-non-conflicting-writes.spec.ts`** — twee contexts, A schrijft `teamName="X"`, B schrijft `tag1Label="Y"`, parallel. Verwacht: beide slagen; beide clients zien de gecombineerde waarde (A's teamName + B's tag1Label) via listener. De Settings-UI houdt daarom expliciet de gewijzigde keys bij; een bestaand Firestore-document krijgt uitsluitend die velden als `setDoc(..., { merge: true })`-patch, terwijl een nog niet bestaand document volledig wordt aangemaakt. Bewijst: er is geen onnodig write-conflict of stil verlies tussen verschillende velden van hetzelfde document.
...
- `v2/src/infrastructure/settings/FirestoreSettingsRepository.ts` — `subscribe()` levert `updatedAt` door (epoch ms); bestaande documenten krijgen alleen gewijzigde settingsvelden als merge-patch zodat niet-conflicterende parallelle writes behouden blijven. Nieuwe documenten blijven volledige schemawrites. Idem voor `updatedAt` bij roster.
