...
5. Verticale flows blijven inhoudelijk in de volgorde instellingen/team → wedstrijdopzet → live wedstrijd → historie → stats → trends. **De eerste flow (instellingen/team) is expliciet een walking-skeleton-valideringspoort**: pas doorgaan naar de overige vijf zodra daar aantoonbaar werken: build, PWA-installeerbaarheid/offline-reload, NL/EN-taalwissel zonder reload, en Vitest + Playwright naast elkaar groen. PR #16 bewijst de basisbuild en beide test-runners; PWA-, opslag- en flowintegratie zijn nog niet bewezen. Na deze eerste flow volgt eerst de afzonderlijke cloud-/sync-/tenancy-beslispoort en de settings/team-platformpilot uit het implementatieplan. Daarmee worden repository-ID's, autorisatie en synchronisatie bewezen voordat de complexe wedstrijdflows worden herschreven. Dit verandert de frontendlaaggrenzen uit dit ADR niet.
