// Bewijst ADR-003's Rules-only uitnodigingsflow end-to-end:
...
describe('bootstrap: eerste organisatie/team zelf aanmaken', () => {
  ...
  it('bootstrap-create faalt zonder correct `uid`-veld', async () => {
    const db = authCtx(env, USERS.grace.uid, { email: USERS.grace.email, email_verified: true });
    const newOrg = 'org-grace-nieuw-2';
    await withAdmin(env, async (adb) => {
      await adb.collection('organizations').doc(newOrg).set({ name: 'Graces Org 2', createdBy: USERS.grace.uid });
    });
    await assertFails(
      setDoc(doc(db, 'organizations', newOrg, 'organizationMembers', USERS.grace.uid), {
        role: 'organizationOwner',
...
describe('uitnodiging accepteren', () => {
  beforeEach(async () => {
    ...
    await withAdmin(env, async (db) => {
      await db.collection('organizations').doc(ORG_A).collection('invitations').doc(INV_ID).set({
        email: USERS.grace.email,
        role: 'viewer',
        status: 'pending',
        invitedBy: USERS.bob.uid,
  ...
  it('jack (niet-geverifieerd) mag NIET accepteren', async () => {
    ...
    await withAdmin(env, async (db) => {
      await db.collection('organizations').doc(ORG_A).collection('invitations').doc(jackInvId).set({
...
describe('membership claimen na acceptatie', () => {
  ...
  it('grace mag membership NIET los aanmaken zonder de uitnodiging atomair te claimen', async () => {
    ...
    await assertFails(
      setDoc(doc(db, 'organizations', ORG_A, 'organizationMembers', USERS.grace.uid), {
        role: 'viewer',
        email: USERS.grace.email,
        uid: USERS.grace.uid,
        invitationId: INV_ID,
  ...
  it("grace mag het `uid`-veld NIET vervalsen bij het claimen (issue #28-integriteit)", async () => {
    const db = authCtx(env, USERS.grace.uid, {
      email: USERS.grace.email,
      email_verified: true,
    });
    ...
    batch.set(doc(db, 'organizations', ORG_A, 'organizationMembers', USERS.grace.uid), {
      role: 'viewer',
      email: USERS.grace.email,
      uid: USERS.bob.uid, // vervalst — moet gelijk zijn aan eigen uid
      invitationId: INV_ID,
      joinedAt: new Date(),
    });
    batch.update(doc(db, 'organizations', ORG_A, 'invitations', INV_ID), {
...
describe('ingetrokken uitnodiging blokkeert claim', () => {
  it('henry (ingetrokken uitnodiging) kan GEEN membership aanmaken', async () => {
    await withAdmin(env, async (db) => {
      await db.collection('organizations').doc(ORG_A).collection('invitations').doc('inv-henry').set({
...
describe('uitgebruikte uitnodiging blokkeert herinstroom (replay-blokkade)', () => {
  it('grace kan membership niet opnieuw aanmaken na claim → owner-delete → her-claim', async () => {
    ...
    await assertSucceeds(deleteDoc(doc(aliceDb, 'organizations', ORG_A, 'organizationMembers', USERS.grace.uid)));
