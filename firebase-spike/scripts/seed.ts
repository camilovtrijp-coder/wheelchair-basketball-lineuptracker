...
initializeApp({
  ...
  credential: cert({
    projectId: 'demo-lineup-tracker-spike',
    clientEmail: 'seed@demo-lineup-tracker-spike.iam.gserviceaccount.com',
...
const SEED_PLAYERS_B = [
  { id: 1, nr: '5',  naam: 'Fictief NBB Speler', kl: '2.0', vrouw: false, jeugd: false },
];
...
async function seed(): Promise<void> {
  ...
  await Promise.all(
    Object.values(USERS).map((u) =>
      createUser(u.uid, u.email, u.password, u.verified),
    ),
  );
  ...
  await memberRef(orgA, USERS.alice.uid).set({ role: 'organizationOwner', email: USERS.alice.email, joinedAt: now });
  await memberRef(orgA, USERS.bob.uid).set({ role: 'organizationAdmin', email: USERS.bob.email, joinedAt: now });
  ...
  // Uitnodigingen org A — dekken alle randgevallen voor invitation-flow.spec.ts:
  ...
  // alice = viewer in org B (dezelfde alice die owner is in org A)
  await memberRef(orgB, USERS.frank.uid).set({ role: 'organizationOwner', email: USERS.frank.email, joinedAt: now });
  await memberRef(orgB, USERS.alice.uid).set({ role: 'viewer', email: USERS.alice.email, joinedAt: now });
