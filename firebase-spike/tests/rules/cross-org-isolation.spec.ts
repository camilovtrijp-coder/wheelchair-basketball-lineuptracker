// Bewijst item 5 (cross-org isolatie):
...
beforeEach(async () => {
  ...
  await withAdmin(env, async (db) => {
    ...
    await db.collection('organizations').doc(ORG_A)
      ...
      .collection('teamMembers').doc(USERS.dave.uid)
    ...
    await db.collection('organizations').doc(ORG_B)
      .collection('teams').doc(TEAM_B1).set({ name: 'Team B1', createdBy: USERS.frank.uid });
    await db.collection('organizations').doc(ORG_B)
...
describe('dave (lid van org A) probeert org B te lezen of schrijven', () => {
  it('mag settings van org B NIET lezen (geen membership)', async () => {
    ...
    await assertFails(
      getDoc(doc(db, 'organizations', ORG_B, 'teams', TEAM_B1, 'settings', 'current')),
    );
  });
  ...
  it('mag roster van org B NIET lezen (geen membership)', async () => {
    const db = authCtx(env, USERS.dave.uid, { email: USERS.dave.email });
    await assertFails(
      getDoc(doc(db, 'organizations', ORG_B, 'teams', TEAM_B1, 'roster', 'current')),
    );
  });
  ...
  it('mag settings van org B NIET schrijven', async () => {
    const db = authCtx(env, USERS.dave.uid, { email: USERS.dave.email });
    await assertFails(
      setDoc(doc(db, 'organizations', ORG_B, 'teams', TEAM_B1, 'settings', 'current'), SAMPLE_SETTINGS),
    );
  });
  ...
  it('mag eigen org A-settings wel lezen (controlegroep)', async () => {
    ...
    await assertSucceeds(
      getDoc(doc(db, 'organizations', ORG_A, 'teams', TEAM_A1, 'settings', 'current')),
    );
  });
});
